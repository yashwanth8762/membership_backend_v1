    ...
    if (existingForm) {
      ...
      const defaultForm = await MembershipForm.create({
        fields: [
          ...
          { inputType: 'text', label: 'DOB', label_kn: 'ಹುಟ್ಟಿದ ದಿನಾಂಕ', required: true, order: 4 },
          { inputType: 'text', label: 'Caste', label_kn: 'ಜಾತಿ', required: false, order: 5 },
          { inputType: 'text', label: 'Qualification', label_kn: 'ಅರ್ಹತೆ', required: false, order: 6 },
          { inputType: 'textarea', label: 'ವಿಳಾಸ / Address', label_kn: 'ವಿಳಾಸ', required: true, order: 7 },
          { inputType: 'text', label: 'Mobile Number', label_kn: 'ಮೊಬೈಲ್ ಸಂಖ್ಯೆ', required: true, order: 8 },
          { inputType: 'text', label: 'Assembly Constituency', label_kn: 'ವಿಧಾನಸಭಾ ಕ್ಷೇತ್ರ', required: false, order: 9 },
          { inputType: 'number', label: 'Amount', label_kn: 'ಮೊತ್ತ', required: false, order: 10 },
          { inputType: 'media', label: 'Photos', label_kn: 'ಫೋಟೋಗಳು', required: false, order: 11 },
        ],
      });
      formId = defaultForm._id;
      console.log(`✓ Created default form: ${formId}`);
...
(async () => {
  await connectDB();
  const filePath = './bangaluru1.xlsx';
